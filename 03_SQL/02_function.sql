...
select emp_id,
       ...
       floor(salary * 1.15) as "SAL_RAISE"
from emp;
...
select curdate(), -- date type
        ...
        month(curdate()),
        day(curdate()),
        dayofweek(curdate()); -- 요일: 1:일요일~7: 토요일
-- 시간 타입에서 시 분 초 조회
select curtime(),
		hour(curtime()),
        minute(curtime()),
...
select hire_date,
	   subdate(hire_date, interval 10 day) "입사 10일 전",
	   adddate(hire_date, interval 10 day) "입사 10일 후"
from emp
where dept_name = 'IT';

select hire_date,
	   subdate(hire_date, interval 10 quarter) "입사 10분기 전",
	   adddate(hire_date, interval 10 month) "입사 10개월 후"
       adddate(hire_date, interval 10 week) "입사 10주 후"
from emp
where dept_name = 'IT';
...
select emp_id,
       ...
       job,
       dept_name,
       if(dept_name is null, '부서미배치', dept_name)
from emp
order by dept_name;
...
from emp;
-- comm_pct가 없으면 " 커미션없음"을 있으면 "커미션있음"으로출력
select if(comm_pct is null, '커미션없음', '커미션있음') from emp;
...
***********************************************/
select '10' + '20';
select datediff(curdate(), '2025-03-20');
