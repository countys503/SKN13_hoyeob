...
-- 전체 직원들 중 담당 업무 ID(emp.job_id) 가 'ST_CLERK'인 직원들의 평균 급여보다 
-- 적은 급여를 받는 직원들의 모든 정보를 조회. 
...
--  부서 위치(dept.loc) 가 'New York'인 부서에 소속된 직원의 ID(emp.emp_id), 이름(emp.emp_name),
...
-- 전체 직원들중 부서_ID(emp.dept_id)가 20인 부서의 모든 직원들 보다 급여(emp.salary)를 
-- 많이 받는 직원들의 정보를 sub query를 이용해 조회.
select * from emp
