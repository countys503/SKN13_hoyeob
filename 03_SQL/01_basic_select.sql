...
-- 커미션_PCT(comm_pct), 급여에 커미션_PCT를 곱한 값을 조회.
select emp_id,
	   emp_name,
       salary,
       comm_pct,
...
-- 직원 이름(emp_name)과 salary를 조회. salart앞에 '$'를 붙여서 출력.
select emp_name,
	   concat('$',salary, '달러') as "salary"
from emp;
...
from emp
where year(hire_date) = 2004; -- year(날짜) : 년도만 추출
...
-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중에서 'Marketing' 이나 'Sales' 부서에 소속된 
...
order by 3 desc; -- select의 컬럼 순번으로 기준 컬럼을 지정.
