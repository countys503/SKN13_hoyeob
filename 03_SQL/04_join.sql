...
select e.emp_id, e.emp_name, m.emp_name as "manage name"
...
select * from emp where dept_ in null
salct count(*) from emp;
-- select count(*)
select e.emp_id, e.emp_name, e.salary,
	   d.dept_name, d.loc
...
select e.emp_id,
	   e.emp_name,
       ifnull(j.job_title, '미배정') as "job title"
...
-- EMP 테이블에서 부서_ID(emp.dept_id)가 90 인 모든 직원들의 id(emp.emp_id), 이름(emp.emp_name), 
...
-- 2003년~2005년 사이에 입사한 모든 직원의 id(emp.emp_id), 이름(emp.emp_name), 업무명(job.job_title), 
-- 급여(emp.salary), 입사일(emp.hire_date),
